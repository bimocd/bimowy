...
function LeftBottomExerciseButton({ exercise, i }: { i: number; exercise: ExerciseInstance }) {
	...
	return (
        ...
        disabled:grayscale-50`}
			...
			disabled={pageState === PageState.Loading || exercise.state === ExerciseState.Correcting}
			onClick={(ev) => {
...
function LeftBottomExerciseButtonEmoji({
	...
	const isAllCorrect = inputs.every((inp) => inp.correction.corrected && inp.correction.correct);
...
function RightBottomBar() {
	const [
		...
		useExerciseStore((state) => state.currentIndex === state.exercises.length - 1),
